...
public:
    ...
    static ExtendShiftType *get(ExtendType extendType, int64_t lo = 0, int64_t hi = 4) {
        // 对于我们目前遇到的情况，默认的IMM在[0, 4]之间。
...
const unordered_multimap<InstId, InstType*> legal_inst_type_multimap = {
        ...
        {Inst::kIdAdd, InstType::get(REG_GPXZ, REG_GPXZ, REG_GPXZ, ExtendShiftType::get(ExtendType::SXTW))},
...
考虑到对于前端的处理，因此这里的处理是比较容易的。需要考虑到的是，在`IMM`缺失时，设置立即数为0（默认值），也就是`inst->setOp(op_id, Imm(asmjit::arm::ShiftOp::kLSL, 0));`。
